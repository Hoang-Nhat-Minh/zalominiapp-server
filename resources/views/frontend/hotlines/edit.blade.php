...
@section('title', 'Chỉnh sửa số Hotline - Hệ thống Quản Trị')
...
@section('content')
    <main class="admin-content-wrapper hotlines-wrapper">
        <div class="hotlines-header" style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 24px;">
            <div>
                <h1 style="font-size: 24px; font-weight: 600; color: var(--text-main); margin-bottom: 4px;">Chỉnh sửa đường dây nóng: {{ $hotline->name }}</h1>
                <p style="color: var(--text-muted); font-size: 14px;">Cập nhật thông tin liên hệ khẩn cấp hiển thị cho công dân.</p>
            </div>
            ...
            <form action="{{ route('hotlines.update', $hotline->id) }}" method="POST">
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 16px; margin-bottom: 16px;">
                    <div style="grid-column: span 2;">
                        ...
                        <input type="text" name="name" value="{{ old('name', $hotline->name) }}" required style="width: 100%; padding: 10px 12px; border: 1px solid var(--border); border-radius: 6px; font-size: 14px;">
                    </div>
                    ...
                    <div>
                        ...
                        <select name="category" required style="width: 100%; padding: 10px 12px; border: 1px solid var(--border); border-radius: 6px; font-size: 14px; background: white;">
                            <option value="police" {{ old('category', $hotline->category) === 'police' ? 'selected' : '' }}>Công an / Giữ gìn ANTT</option>
                            <option value="medical" {{ old('category', $hotline->category) === 'medical' ? 'selected' : '' }}>Y tế / Cấp cứu</option>
                            <option value="rescue" {{ old('category', $hotline->category) === 'rescue' ? 'selected' : '' }}>PCCC / Cứu hộ khẩn cấp</option>
                            <option value="tdp" {{ old('category', $hotline->category) === 'tdp' ? 'selected' : '' }}>Tổ trưởng Tổ dân phố</option>
                            <option value="other" {{ old('category', $hotline->category) === 'other' ? 'selected' : '' }}>Khác / Tổng đài tư vấn</option>
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" name="phone" value="{{ old('phone', $hotline->phone) }}" required style="width: 100%; padding: 10px 12px; border: 1px solid var(--border); border-radius: 6px; font-size: 14px;">
                    </div>

                    <div style="grid-column: span 2;">
                        <label style="display: block; font-weight: 600; margin-bottom: 6px; font-size: 14px;">Địa chỉ / Vị trí trụ sở</label>
                        <input type="text" name="address" value="{{ old('address', $hotline->address) }}" style="width: 100%; padding: 10px 12px; border: 1px solid var(--border); border-radius: 6px; font-size: 14px;">
                    </div>

                    <div style="grid-column: span 2;">
                        <label style="display: block; font-weight: 600; margin-bottom: 6px; font-size: 14px;">Mô tả chi tiết / Lịch trực ban</label>
                        <textarea name="description" rows="3" style="width: 100%; padding: 10px 12px; border: 1px solid var(--border); border-radius: 6px; font-size: 14px;">{{ old('description', $hotline->description) }}</textarea>
                    </div>

                    <div>
                        <label style="display: block; font-weight: 600; margin-bottom: 6px; font-size: 14px;">Thứ tự hiển thị</label>
                        <input type="number" name="order" value="{{ old('order', $hotline->order) }}" min="0" style="width: 100%; padding: 10px 12px; border: 1px solid var(--border); border-radius: 6px; font-size: 14px;">
                    </div>

                    <div style="display: flex; align-items: center; margin-top: 24px;">
                        <label style="display: flex; align-items: center; gap: 8px; font-weight: 600; font-size: 14px; cursor: pointer;">
                            <input type="checkbox" name="is_active" value="1" {{ old('is_active', $hotline->is_active) ? 'checked' : '' }} style="width: 18px; height: 18px;">
                ...
                <div style="display: flex; gap: 12px; justify-content: flex-end; margin-top: 24px; border-top: 1px solid var(--border); padding-top: 16px;">
                    ...
                    <button type="submit" class="admin-btn admin-btn-primary" style="padding: 10px 24px; border-radius: 6px; font-weight: 600; border: none; cursor: pointer;">Cập nhật Hotline</button>
                </div>
            </form>
        </div>
    </main>
