...
@section('content')
    <main class="admin-content-wrapper school-create-wrapper" style="padding: 24px;">
        <div style="margin-bottom: 24px; display: flex; align-items: center; gap: 12px;">
            ...
            <div>
                <h1 style="font-size: 24px; font-weight: 600; color: var(--text-main); margin: 0;">Thêm cơ sở giáo dục mới</h1>
                <p style="color: var(--text-muted); font-size: 14px; margin: 2px 0 0 0;">Nhập thông tin chi tiết để thêm mới cơ sở giáo dục vào bản đồ tra cứu của người dân.</p>
            </div>
        </div>
            ...
            <form action="{{ route('schools.store') }}" method="POST">
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 16px; margin-bottom: 16px;">
                    <div>
                        ...
                        <input type="text" name="name" value="{{ old('name') }}" placeholder="Nhập tên trường học..." class="e-input" style="width: 100%;" required>
                    </div>
                    <div>
                        ...
                        <select name="level" class="e-input" style="width: 100%; height: 48px;" required>
                            ...
                            <option value="kindergarten" {{ old('level') === 'kindergarten' ? 'selected' : '' }}>Mầm non</option>
                            <option value="primary" {{ old('level') === 'primary' ? 'selected' : '' }}>Tiểu học</option>
                            <option value="secondary" {{ old('level') === 'secondary' ? 'selected' : '' }}>Trung học cơ sở (THCS)</option>
                            <option value="high_school" {{ old('level') === 'high_school' ? 'selected' : '' }}>Trung học phổ thông (THPT)</option>
                            <option value="other" {{ old('level') === 'other' ? 'selected' : '' }}>Khác/Liên cấp</option>
                        </select>
                    </div>
                </div>
                ...
                <div style="margin-bottom: 16px;">
                    ...
                    <input type="text" name="address" value="{{ old('address') }}" placeholder="Số nhà, tên đường, tổ dân phố..." class="e-input" required>
                </div>

                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 16px; margin-bottom: 16px;">
                    <div>
                        <label style="display: block; font-weight: 600; margin-bottom: 6px; color: var(--text-main);">Số điện thoại liên hệ *</label>
                        <input type="text" name="phone" value="{{ old('phone') }}" placeholder="Nhập số điện thoại..." class="e-input" required>
                    </div>
                    <div>
                        <label style="display: block; font-weight: 600; margin-bottom: 6px; color: var(--text-main);">Địa chỉ Email</label>
                        <input type="email" name="email" value="{{ old('email') }}" placeholder="Nhập email liên hệ..." class="e-input">
                    </div>
                </div>

                <div style="margin-bottom: 16px;">
                    <label style="display: block; font-weight: 600; margin-bottom: 6px; color: var(--text-main);">Địa chỉ Website (URL)</label>
                    <input type="url" name="website" value="{{ old('website') }}" placeholder="http://example.edu.vn..." class="e-input">
                </div>

                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 16px; margin-bottom: 16px;">
                    <div>
                        <label style="display: block; font-weight: 600; margin-bottom: 6px; color: var(--text-main);">Vĩ độ GPS (Latitude)</label>
                        <input type="number" step="any" name="latitude" value="{{ old('latitude') }}" placeholder="Ví dụ: 20.9723" class="e-input">
                    </div>
                    <div>
                        <label style="display: block; font-weight: 600; margin-bottom: 6px; color: var(--text-main);">Kinh độ GPS (Longitude)</label>
                        <input type="number" step="any" name="longitude" value="{{ old('longitude') }}" placeholder="Ví dụ: 105.7742" class="e-input">
                    </div>
                </div>

                <div style="margin-bottom: 24px;">
                    <label style="display: block; font-weight: 600; margin-bottom: 6px; color: var(--text-main);">Giới thiệu về trường</label>
                    <textarea name="description" rows="4" placeholder="Nhập nội dung giới thiệu ngắn..." class="e-input" style="resize: none; height: auto;">{{ old('description') }}</textarea>
                </div>
