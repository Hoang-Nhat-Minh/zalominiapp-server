...
@section('title', 'Chỉnh sửa danh mục tin tức - Hệ thống Quản Trị')

@section('content')
    <main class="admin-content-wrapper">
        <div style="margin-bottom: 24px; display: flex; align-items: center; justify-content: space-between;">
            <div>
                <h1 style="font-size: 24px; font-weight: 600; color: var(--text-main); margin-bottom: 4px;">Chỉnh sửa danh mục: {{ $category->name }}</h1>
                <p style="color: var(--text-muted); font-size: 14px;">Cập nhật thông tin chi tiết của danh mục tin tức.</p>
            </div>
        ...
        <div class="gov-card" style="background: var(--surface); padding: 24px; border-radius: 8px; border: 1px solid var(--border); max-width: 700px;">
            <form action="{{ route('news-categories.update', $category->id) }}" method="POST">
                ...
                <div style="margin-bottom: 20px;">
                    ...
                    <input type="text" name="name" value="{{ old('name', $category->name) }}" required style="width: 100%; padding: 10px 14px; border: 1px solid var(--border); border-radius: 6px; outline: none; font-size: 14px;">
                </div>

                <div style="margin-bottom: 20px;">
                    <label style="display: block; font-weight: 600; margin-bottom: 8px; color: var(--text-main);">Mô tả danh mục</label>
                    <textarea name="description" rows="4" style="width: 100%; padding: 10px 14px; border: 1px solid var(--border); border-radius: 6px; outline: none; font-size: 14px; resize: vertical;">{{ old('description', $category->description) }}</textarea>
                </div>

                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 16px; margin-bottom: 24px;">
                    <div>
                        <label style="display: block; font-weight: 600; margin-bottom: 8px; color: var(--text-main);">Thứ tự hiển thị</label>
                        <input type="number" name="order" value="{{ old('order', $category->order) }}" min="0" style="width: 100%; padding: 10px 14px; border: 1px solid var(--border); border-radius: 6px; outline: none; font-size: 14px;">
                    </div>
                ...
                <div style="display: flex; gap: 12px; justify-content: flex-end;">
                    ...
                    <button type="submit" class="admin-btn admin-btn-primary" style="padding: 10px 24px; border-radius: 6px; font-weight: 600; border: none; cursor: pointer;">
                        <i class="ph ph-floppy-disk"></i> Cập nhật danh mục
                    </button>
                </div>
            </form>
        </div>
