...
@section('title', 'Chỉnh sửa cảnh báo thời tiết - Hệ thống Quản Trị')
...
@section('content')
    <main class="admin-content-wrapper weather-wrapper">
        <div class="weather-header" style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 24px;">
            <div>
                <h1 style="font-size: 24px; font-weight: 600; color: var(--text-main); margin-bottom: 4px;">Chỉnh sửa bản tin cảnh báo thời tiết</h1>
                <p style="color: var(--text-muted); font-size: 14px;">Cập nhật thông tin cảnh báo thiên tai hiển thị cho công dân.</p>
            </div>
        ...
        <div class="gov-card" style="background: var(--surface); padding: 24px; border-radius: 8px; border: 1px solid var(--border); max-width: 800px;">
            <form action="{{ route('weather-alerts.update', $alert->id) }}" method="POST">
                @csrf
                @method('PUT')
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 16px; margin-bottom: 16px;">
                    <div style="grid-column: span 2;">
                        ...
                        <input type="text" name="title" value="{{ old('title', $alert->title) }}" required style="width: 100%; padding: 10px 12px; border: 1px solid var(--border); border-radius: 6px; font-size: 14px;">
                    </div>
                    ...
                    <div>
                        ...
                        <select name="level" required style="width: 100%; padding: 10px 12px; border: 1px solid var(--border); border-radius: 6px; font-size: 14px; background: white;">
                            <option value="warning" {{ old('level', $alert->level) === 'warning' ? 'selected' : '' }}>Cảnh báo (Mức vừa)</option>
                            <option value="danger" {{ old('level', $alert->level) === 'danger' ? 'selected' : '' }}>Cực đoan - Nguy hiểm khẩn cấp (Mức đỏ)</option>
                            <option value="info" {{ old('level', $alert->level) === 'info' ? 'selected' : '' }}>Thông tin thời tiết chung (Mức xanh)</option>
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" name="area" value="{{ old('area', $alert->area) }}" required style="width: 100%; padding: 10px 12px; border: 1px solid var(--border); border-radius: 6px; font-size: 14px;">
                    </div>
                    ...
                    <div style="grid-column: span 2;">
                        ...
                        <textarea name="content" rows="5" required style="width: 100%; padding: 10px 12px; border: 1px solid var(--border); border-radius: 6px; font-size: 14px;">{{ old('content', $alert->content) }}</textarea>
                    </div>
                    ...
                    <div style="display: flex; align-items: center; margin-top: 16px; grid-column: span 2;">
                        <label style="display: flex; align-items: center; gap: 8px; font-weight: 600; font-size: 14px; cursor: pointer;">
                            <input type="checkbox" name="is_active" value="1" {{ old('is_active', $alert->is_active) ? 'checked' : '' }} style="width: 18px; height: 18px;">
                ...
                <div style="display: flex; gap: 12px; justify-content: flex-end; margin-top: 24px; border-top: 1px solid var(--border); padding-top: 16px;">
                    ...
                    <button type="submit" class="admin-btn admin-btn-primary" style="padding: 10px 24px; border-radius: 6px; font-weight: 600; border: none; cursor: pointer;">Cập nhật Cảnh báo</button>
                </div>
            </form>
        </div>
    </main>
